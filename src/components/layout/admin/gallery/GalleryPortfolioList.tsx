...
type Props = {
    ...
    onAction?: (portfolioId: string, action: Action) => void;
    visibilityStags:visibilityStag[];
};

export default function GalleryPortfolioListLayout ({gallery, onAction, serviceTypes, serviceSpaceTypes, visibilityStags}: Props) {
    ...
    const derivedServiceTypes = Array.from(
        new Set((gallery || []).map((p) => p.service_type).filter(Boolean))
    );

    const derivedServiceSpaceTypes = Array.from(
        new Set((gallery || []).map((p) => p.service_space_type).filter(Boolean))
    );

    const derivedStages = Array.from(
        new Set((gallery || []).map((p) => p.stage.value).filter(Boolean))
    );
    ...
    // filtering logic
    const filtered = gallery.filter((portfolio) => {
        // query match against title/description
        const q = query.trim().toLowerCase();
            if (q) {
                const hay = `${portfolio.title} ${portfolio.description || ""}`.toLowerCase();
                if (!hay.includes(q)) return false;
            }

            // service type
            if (serviceTypeFilter !== "all") {
                if (portfolio.service_type !== serviceTypeFilter) return false;
            }

            // service space type
            if (serviceSpaceTypeFilter !== "all") {
                if (portfolio.service_space_type !== serviceSpaceTypeFilter) return false;
            }

            // service space type
            if (visibilityStagFilter !== "all") {
                if (portfolio.stage.value !== visibilityStagFilter) return false;
            }
            ...
            <div className="flex max-xl:flex-col items-center gap-2 mb-2 sm:mb-4 w-full h-fit">
                        ...
                        {sorted.map((portfolio) => (
                            <>
                                <li className="flex gap-2">
                                    <Link to={PATHS.ADMIN.galleryPortfolioUpdate('slug')} className="flex max-xs:flex-col gap-2 w-full">
                                        <div className="xs:min-w-max xs:h-28 aspect-video overflow-hidden">
                                            <ZoomImage src={portfolio.src} alt="" className="object-cover h-full w-full rounded-lg" />
                                        </div>

                                        <div className="flex gap-2 w-full">
                                            <div className="flex flex-col gap-2 w-full">

                                                <h4 className="text-ellipsis-2line md:text-ellipsis-1line font-medium text-sm md:text-lg text-muted"> {portfolio.title} </h4>

                                                <div className="hidden sm:flex flex-wrap gap-1 sm:gap-2 w-fit">
                                                    <span className="text-ellipsis-1line text-2xs px-2 py-1 border border-muted/15 rounded-full bg-surface">{portfolio.service_type}</span>
                                                    <span className="text-ellipsis-1line text-2xs px-2 py-1 border border-muted/15 rounded-full bg-surface">{portfolio.service_space_type}</span>
                                                </div>

                                                <div className="flex flex-wrap">
                                                    <span className="text-2xs min-w-max after:content-['•'] after:mx-1 last:after:content-none">{portfolio.stage.label}</span>
                                                    <span className="text-2xs min-w-max after:content-['•'] after:mx-1 last:after:content-none">{portfolio.date}</span>
                                                </div>
                                            ...
                                            <div className="w-fit">
                                                <button
                                                    type="button"
                                                    aria-haspopup="menu"
                                                    aria-expanded={openId === portfolio.id}
                                                    ref={triggerRef}
                                                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleMenu(portfolio.id); }}
                                                    ...
                                                    {openId === portfolio.id && (
                                                        ...
                                                        >
                                                            <button role="menuitem"
                                                                onClick={() => handleAction(portfolio.id, "edit")}
                ...
                :
                    <div className="flex items-center justify-center gap-2 w-full h-full border border-red-400">
                        ...
                        <p>
                            No portfolios match your current search or filters.
                        </p>
